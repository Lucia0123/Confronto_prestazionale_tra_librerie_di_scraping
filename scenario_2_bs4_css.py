...
def ottieni_e_pulisci_stringa(tag: Tag):
    stringa = tag.text.replace("\n", "").replace("\xa0", "")
    return stringa
...
corpo_tabella = soup.select_one("tbody")
righe = corpo_tabella.select("tr")
...
for riga in righe:
    ...
    dati = {
        ...
        "paese": ottieni_e_pulisci_stringa(tds[6])
        }
    print(dati)
