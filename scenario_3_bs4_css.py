...
while current_url:
    ...
    libri = soup.select_one("ol.row").select("li")
    for l in libri:
        libro = {
            "titolo" : l.select_one("h3 a")["title"],
            "prezzo" : l.select_one(".price_color").text
        }
        print(libro)

    # trovo il link alla prossima pagina
    bottone_next = soup.select_one("li.next")
    if bottone_next:
        current_url = urljoin(base_url, bottone_next.select_one("a")["href"])
    else:
        current_url = None
