...
while current_url:
    ...
    libri = selector.css("ol.row")[0].css("li")
    for l in libri:
        libro = {
            "titolo" : l.css("h3 a::attr(title)").get(),          
            "prezzo" : l.css(".price_color::text").get().replace("Â", "")
        }
        print(libro)

    # trovo il link alla prossima pagina
    bottone_next = selector.css("li.next")
    if bottone_next:
        current_url = urljoin(base_url, bottone_next.css("a::attr(href)").get())
    else:
        current_url = None
